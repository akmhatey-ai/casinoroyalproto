...
export async function GET() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { username: true, name: true, bio: true, website: true, twitter: true, github: true, profilePublic: true, image: true, walletSolana: true, walletEvm: true },
  });
...
export async function PATCH(req: Request) {
  ...
  const { username, bio, website, twitter, github, profilePublic } = body as {
    username?: string; bio?: string; website?: string; twitter?: string; github?: string; profilePublic?: boolean;
  };
